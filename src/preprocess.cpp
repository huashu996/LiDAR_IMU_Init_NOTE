...
void Preprocess::process_cut_frame_livox(const livox_ros_driver::CustomMsg::ConstPtr &msg,
    ...
    int plsize = msg->point_num;  // 点云数据中的点数
    pl_surf.clear();  // 清空平面点云容器
    pl_surf.reserve(plsize);  // 预留平面点云容器的空间
    pl_full.clear();  // 清空完整点云容器
    pl_full.resize(plsize);  // 调整完整点云容器的大小为点云数据点数
    int valid_point_num = 0;  // 有效点数计数器

    for (uint i = 1; i < plsize; i++) { //对于每个点
        if ((msg->points[i].line < N_SCANS) &&
        ((msg->points[i].tag & 0x30) == 0x10 || (msg->points[i].tag & 0x30) == 0x00)) //根据激光雷达数据中的标签信息和线号进行有效点云的筛选。
        { 
            valid_point_num++;
            if (valid_point_num % point_filter_num == 0) { //通过控制点的采样频率来进行筛选保存到pl_full
                ...
                if (dist < blind * blind) continue;  // 距离过近的点将被忽略

                if ((abs(pl_full[i].x - pl_full[i - 1].x) > 1e-7)
                    || (abs(pl_full[i].y - pl_full[i - 1].y) > 1e-7)
                    || (abs(pl_full[i].z - pl_full[i - 1].z) > 1e-7)) {
                    pl_surf.push_back(pl_full[i]); //在点云数据中去除连续重复的点，只保留非重复的点，加入平面点云容器
                }
            }
        }
    }
    sort(pl_surf.points.begin(), pl_surf.points.end(), time_list_cut_frame);  // 根据时间对平面（非重复）点云进行排序
    //end time of last frame，单位ms
    // 更新上一帧的结束时间 单位：毫秒
    double last_frame_end_time = msg->header.stamp.toSec() * 1000;
    uint valid_num = 0; //用于记录有效非重复点的数量
    uint cut_num = 0; //用于记录切割的帧数
    uint valid_pcl_size = pl_surf.points.size(); //获取平面点云容器 pl_surf 中的点的数量。

    int required_cut_num = required_frame_num; //根据所需的帧数，初始化要切割的帧数。
    if (scan_count < 5) //前5个scan不做切割
        required_cut_num = 1;
    ...
    for (uint i = 1; i < valid_pcl_size; i++) {
        ...
        pcl_cut.push_back(pl_surf[i]); // 将临时将要切割点加入pcl_cut点云容器
        if (valid_num == (int((cut_num + 1) * valid_pcl_size / required_cut_num) - 1)) { //当数量到一定值，对pcl_cut容器中的点云赋给pcl_out，并将每一部分的切割时间保存到time_lidar以区分
            cut_num++; //增加切割的帧数计数
            time_lidar.push_back(last_frame_end_time);  //将上一帧结束时间加入时间戳容器 time_lidar。
            PointCloudXYZI::Ptr pcl_temp(new PointCloudXYZI()); //Initialize shared_ptr 创建一个新的共享指针 pcl_temp，用于存储切割后的点云数据。
            *pcl_temp = pcl_cut;
            pcl_out.push_back(pcl_temp);  //将切割后的点云共享指针 pcl_temp 加入输出点云容器 pcl_out。
            //Update frame head
            last_frame_end_time += pl_surf[i].curvature; //更新上一帧的结束时间，加上当前点的曲率值。
            pcl_cut.clear();  //清空临时点云容器
            pcl_cut.reserve(valid_pcl_size * 2 / required_frame_num); //为临时点云容器 pcl_cut 分配新的空间，以适应下一帧的点云数据。
        }
    }
}
