...
void LI_Init::push_ALL_IMU_CalibState(const sensor_msgs::Imu::ConstPtr &msg, const double &mean_acc_norm) {
    CalibState IMUstate;  //创建IMUstate用于储存IMU数据校准
    IMUstate.ang_vel = V3D(msg->angular_velocity.x, msg->angular_velocity.y, msg->angular_velocity.z); //接收角速度信息
    IMUstate.linear_acc =
            V3D(msg->linear_acceleration.x, msg->linear_acceleration.y, msg->linear_acceleration.z) / mean_acc_norm *
            G_m_s2; //接收线速度信息
    /*
	double linear_acc_x = IMUstate.linear_acc.x();
	double linear_acc_y = IMUstate.linear_acc.y();
	double linear_acc_z = IMUstate.linear_acc.z();
    printf("Linear Acceleration: x = %f, y = %f, z = %f\n", linear_acc_x, linear_acc_y, linear_acc_z);
    */
    IMUstate.timeStamp = msg->header.stamp.toSec(); //时间戳
    IMU_state_group_ALL.push_back(IMUstate); //将矫正后的IMU数据储存到IMU_state_group_ALL
}
